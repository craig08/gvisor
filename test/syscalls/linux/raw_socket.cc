...
TEST_P(RawSocketTest, ShutdownReadNoop) {
  ...
  size_t read_size = sizeof(kBuf) + HdrLen();
  char *c = static_cast<char*>(malloc(read_size));
  ASSERT_THAT(read(s_, &c, sizeof(c)), SyscallSucceedsWithValue(read_size));
  free(c);
}
...
TEST_P(RawSocketTest, SendAndReceive) {
  ...
  size_t recv_buf_size = sizeof(kBuf) + HdrLen();
  char *recv_buf = static_cast<char*>(malloc(recv_buf_size));
  ASSERT_NO_FATAL_FAILURE(ReceiveBuf(recv_buf, recv_buf_size));
  EXPECT_EQ(memcmp(recv_buf + HdrLen(), kBuf, sizeof(kBuf)), 0);
  free(recv_buf);
}
...
TEST_P(RawSocketTest, MultipleSocketReceive) {
  ...
  size_t recv_buf_size = sizeof(kBuf) + HdrLen();
  char *recv_buf1 = static_cast<char*>(malloc(recv_buf_size));
  ASSERT_NO_FATAL_FAILURE(ReceiveBuf(recv_buf1, recv_buf_size));
  free(recv_buf1);

  // Receive it on socket 2.
  char *recv_buf2 = static_cast<char*>(malloc(recv_buf_size));
  ASSERT_NO_FATAL_FAILURE(RecvNoCmsg(s2, recv_buf2, recv_buf_size));
  free(recv_buf2);

  EXPECT_EQ(memcmp(recv_buf1 + HdrLen(),
                   recv_buf2 + HdrLen(), sizeof(kBuf)),
            0);
