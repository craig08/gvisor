...
TEST_F(WriteTest, WriteNormal) {
  ...
  EXPECT_THAT(write(fd.get(), buf.data(), n_write), SyscallSucceedsWithValue(n_write));
  ...
  auto iov_in_write = FuseGenerateIovecs(in_header_write, in_payload_write, payload_buf);
  GetServerActualRequest(iov_in_write);

  EXPECT_EQ(in_header_write.len, sizeof(in_header_write) + sizeof(in_payload_write));
...
TEST_F(WriteTest, WriteShort) {
  ...
  std::vector<char> buf(n_write);
  RandomizeBuffer(buf.data(), buf.size());
  EXPECT_THAT(write(fd.get(), buf.data(), n_write), SyscallSucceedsWithValue(n_written));
...
TEST_F(WriteTest, WriteShortZero) {
  ...
  // Prepare for the write.
  const int n_write = 10, n_written = 0;
  struct fuse_out_header out_header_write = {
      .len = sizeof(struct fuse_out_header) + sizeof(struct fuse_write_out),
  };
  struct fuse_write_out out_payload_write = {
...
TEST_F(WriteTestSmallMaxWrite, WriteSmallMaxWrie) {
  ...
  // Open to get a fd.
  // size+1 to avoid the get_attr. For ideal implementation, will not issue extra get_attr.
  // For current implementation, we always issue a get_attr to fetch the latest inode_size.
  SetServerInodeLookup(test_file_, S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO, n_write+1);
  const int open_flag = O_RDWR;
  struct fuse_out_header out_header_open = {
      .len = sizeof(struct fuse_out_header) + sizeof(struct fuse_open_out),
  };
