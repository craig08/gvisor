...
TEST_F(ReadlinkTest, ReadSymLink) {
  ...
  struct fuse_out_header out_header = {
    .len = static_cast<uint32_t>(sizeof(struct fuse_out_header)) + static_cast<uint32_t>(target_file.length()) + 1,
  };
  std::vector<char> link (target_file.begin(), target_file.end());
  link.push_back(0);
  iov_out = FuseGenerateIovecs(out_header, link);
  SetServerResponse(FUSE_READLINK, iov_out);
  const std::string actual_link = ASSERT_NO_ERRNO_AND_VALUE(ReadLink(symlink_path));
  EXPECT_EQ(0, memcmp(actual_link.c_str(), link.data(), link.size()));

  // next readlink should have link cached, so shouldn't have new request to test server.
...
TEST_F(ReadlinkTest, NotSymlink) {
  ...
  ASSERT_THAT(readlink(testFilePath.c_str(), buf.data(), PATH_MAX), SyscallFailsWithErrno(EINVAL));
}
