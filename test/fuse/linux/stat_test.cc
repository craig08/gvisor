...
class StatTest : public FuseTest {
 public:
  ...
  // StatTest doesn't care the release request when close a fd,
 ...
 protected:
  ...
  PosixErrorOr<FileDescriptor> OpenPath(const std::string &path) {
    ...
    auto res = Open(path.c_str(), open_flag);
    if (res.ok()) {
...
TEST_F(StatTest, FstatNormal) {
  ...
  auto fd = ASSERT_NO_ERRNO_AND_VALUE(OpenPath(test_file_path_));
...
TEST_F(StatTest, StatByFileHandle) {
  ...
  // will be issued after read completes.
